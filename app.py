...
with tab1:
    ...
    with c2:
        ...
        with st.expander("Varsayılan değerleri gör / değiştir", expanded=True):
            ...
            default_er90 = st.number_input("energy_rate_90d [auto]", key="energy_rate_90d_key", format="%.6e")
...
with tab2:
    ...
    with c2:
        roll30_count = st.number_input("Son 30 gündeki deprem sayısı", key="roll30_count_key", format="%.0f")
        roll30_maxmag = st.number_input("Son 30 gündeki maks. büyüklük", key="roll30_maxmag_key", format="%.2f")
        roll30_meanmag = st.number_input("Son 30 gündeki ort. büyüklük", key="roll30_meanmag_key", format="%.2f")
        roll30_depth = st.number_input("Son 30 gündeki ort. derinlik", key="roll30_depth_key", format="%.2f")
        ...
        src_map = {
            "cell": "aynı hücre",
            "neighbor_cell_nearest": "komşu hücre (en yakın)",
            "radius": "yakın çevre (radius)"
        }
        st.caption(f"30 günlük değerler katalogdan otomatik dolduruldu (kaynak: **{src_map.get(src, src)}**).")
        st.caption(f"Ek: roll30_energy_30d = **{roll30_energy:.3e}**, roll30_energy_rate_30d = **{roll30_energy_rate:.3e}**")
